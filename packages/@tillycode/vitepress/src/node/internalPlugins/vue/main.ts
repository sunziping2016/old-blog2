...
export async function transformMain(
  ...
  const prevDescriptor = getPrevDescriptor(filename, variant)
  ...
  // styles
  const stylesCode = await genStyleCode(descriptor, variant, pluginContext)
  ...
  return {
    ...
    map: resolvedMap
      ? {
          ...resolvedMap,
          version: parseInt(resolvedMap.version, 10)
        }
      : {
          mappings: ''
        }
...
async function genStyleCode(
  ...
  if (descriptor.styles.length) {
    for (let i = 0; i < descriptor.styles.length; i++) {
      ...
      if (style.src) {
        await linkSrcToDescriptor(style.src, variant, descriptor, pluginContext)
      }
...
async function genCustomBlockCode(
  ...
  for (let index = 0; index < descriptor.customBlocks.length; index++) {
    ...
    if (block.src) {
      await linkSrcToDescriptor(block.src, variant, descriptor, pluginContext)
    }
...
async function linkSrcToDescriptor(
  ...
  // version query.
  setDescriptor(srcFile.replace(/\?.*$/, ''), variant, descriptor)
}
